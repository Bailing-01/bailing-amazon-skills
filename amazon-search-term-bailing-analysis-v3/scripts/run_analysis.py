#!/usr/bin/env python3
"""
亚马逊搜索词展示量份额报告 Bailing 分析 V3
主分析脚本：读取Excel → 提取层级结构 → 帕累托分析 → C2深度分析 → 输出JSON数据
...
def deep_c2_analysis(portfolio_data):
    """V3: C2 深度分析 — 点击次数分布 + 词根关联分析"""
    results = {}
    for portfolio, p_data in portfolio_data['portfolios'].items():
        terms = p_data.get('search_terms', [])

        # Separate converters from C2 terms
        converters = []
        c2_terms = []
        for t in terms:
        ...
        if not c2_terms:
            continue  # Skip portfolios with no C2 terms

        # C2 click distribution
        c2_1click = [t for t in c2_terms if t.get('clicks', 0) == 1]
        c2_2_5 = [t for t in c2_terms if 2 <= t.get('clicks', 0) <= 5]
        c2_6plus = [t for t in c2_terms if t.get('clicks', 0) >= 6]

        # Extract word roots from converters
        converter_roots = set()
        for t in converters:
            kw = (t.get('keyword') or '').lower()
            words = re.findall(r'[a-zA-Z]{3,}', kw)
            converter_roots.update(words)

        # Classify C2 terms by word root overlap
        shared_root_terms = []
        unique_root_terms = []
        for t in c2_terms:
            kw = (t.get('keyword') or '').lower()
            words = set(re.findall(r'[a-zA-Z]{3,}', kw))
            if not words:
                # Pure ASINs or non-English: no 3+ char English words -> unique_root
                unique_root_terms.append(t)
            elif words & converter_roots:
                shared_root_terms.append(t)
            else:
                unique_root_terms.append(t)

        # Build top 10 shared-root C2 terms (sorted by clicks desc)
        shared_root_terms.sort(key=lambda x: x.get('clicks', 0), reverse=True)
        top_shared = []
        for t in shared_root_terms[:10]:
            kw = (t.get('keyword') or '').lower()
            words = set(re.findall(r'[a-zA-Z]{3,}', kw))
            matched_roots = sorted(words & converter_roots)
            top_shared.append({
                'keyword': t.get('keyword', ''),
                'clicks': int(t.get('clicks', 0)),
                'spend': round(t.get('spend', 0), 2),
                'matched_roots': matched_roots,
            })

        # Build top 10 unique-root C2 terms (sorted by clicks desc)
        unique_root_terms.sort(key=lambda x: x.get('clicks', 0), reverse=True)
        top_unique = []
        for t in unique_root_terms[:10]:
            top_unique.append({
                'keyword': t.get('keyword', ''),
                'clicks': int(t.get('clicks', 0)),
                'spend': round(t.get('spend', 0), 2),
        ...
        # Top converter root words (for reference)
        root_counter = {}
        for t in converters:
            kw = (t.get('keyword') or '').lower()
            words = re.findall(r'[a-zA-Z]{3,}', kw)
            for w in words:
                root_counter[w] = root_counter.get(w, 0) + 1
        top_roots = sorted(root_counter.items(), key=lambda x: x[1], reverse=True)[:20]

        results[portfolio] = {
            'c2_total': len(c2_terms),
            'c2_1click_count': len(c2_1click),
            'c2_1click_spend': round(sum(t.get('spend', 0) for t in c2_1click), 2),
            'c2_2_5_count': len(c2_2_5),
            'c2_2_5_spend': round(sum(t.get('spend', 0) for t in c2_2_5), 2),
            'c2_6plus_count': len(c2_6plus),
            'c2_6plus_spend': round(sum(t.get('spend', 0) for t in c2_6plus), 2),
            'shared_root_count': len(shared_root_terms),
            'shared_root_spend': round(sum(t.get('spend', 0) for t in shared_root_terms), 2),
            'unique_root_count': len(unique_root_terms),
            'unique_root_spend': round(sum(t.get('spend', 0) for t in unique_root_terms), 2),
            'top_shared': top_shared,
            'top_unique': top_unique,
            'converter_root_words': [r[0] for r in top_roots],
        }
...
def main():
    ...
    if args.step in ('search_terms', 'all'):
        ...
        if args.step == 'all':
            ...
            # V3: Deep C2 analysis
            c2_result = deep_c2_analysis(result)
            for p_name, c2_data in c2_result.items():
                if p_name in result.get('portfolios', {}):
                    result['portfolios'][p_name]['c2_deep_analysis'] = c2_data
            output_data = result
        else:
